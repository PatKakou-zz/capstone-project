...
glmnet_pred <- as_factor(ifelse(predict(model_glmnet, test.set, type = "prob")[,"yes"] >= 0.6, "yes", "no"))
...
```{r}
roc_imp <- filterVarImp(x = train.set[, -ncol(train.set)], y = train.set$is_open)
head(roc_imp, 10)
```

```{r}
varImp(model_glmnet, scale = FALSE)
```


```{r}
rf_pred <- predict(model_rf, newdata = test.set)
rf_pred <- as_factor(ifelse(predict(model_rf, test.set, type = "prob")[,"yes"] >= 0.55, "yes", "no"))

confusionMatrix(rf_pred, test.set$is_open)#, mode = "prec_recall")#
```
...
result_obj <- lift(is_open ~ log + rf, data = results)
plot(result_obj, values = 70, auto.key = list(columns = 3,
                                            lines = TRUE,
                                            points = FALSE))
```
...
```{r}
trellis.par.set(caretTheme())
cal_obj <- calibration(is_open ~ log + rf,
                       data = results,
                       cuts = 13)
