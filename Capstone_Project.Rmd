...
I will approximate the type of businesses I mentinned above by selecting businesses that include the tags "restaurants",  "coffee & tea" and "bakeries" in the Categories variable. 
...
Only the "neighborhood' variable have missing value, and it is about 18%. I will get the neighborhood by join the business data to the toronto fsa data. I got that data from the following link: <https://en.wikipedia.org/wiki/List_of_postal_codes_of_Canada:_M>.
...
del_cols= c('business_id', 'name', 'postal_code', 'categories')#, "Borough", "latitude", "longitude")
data = business %>% select(-del_cols)
glimpse(data)
```
...
data_index <- createDataPartition(data$is_open, p = .7, list = FALSE, times = 1)
...
model_glmnet <- train(
  ...
  metric = "ROC",
  method = "glmnet",
  tuneGrid = expand.grid(alpha = 0:1, lambda = seq(0.0001, 0.1, length = 10)),
  trControl = myControl
)
```
