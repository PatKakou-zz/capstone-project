...
Let visualize if Businesses "closed" received less reviews, and if the closure depend on rating. 
...
stars <- business %>%
          ...
          group_by(stars, review_count) %>%
          (summarise(review_count))

p1 <- ggplotly(
        ggplot(business, mapping = aes(stars, review_count)) +
...
```{r}
distribution against business status (open/closed)
