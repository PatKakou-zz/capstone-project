...
```{r}
#Create a new variable containing the number of word in reviews
review <- review %>%
...
Now let check the business data set and respond to those questions:
...
Businesses "closed" received less reviews, and does not depend of rating. The graph below confirms that assumption. Same distribution of rating, may be the closure is not related to the rating.



```{r}
p <- ggplot(business, mapping = aes(stars, review_count, fill = is_open)) +
  geom_col(position = "dodge") +
  labs( x = "Star rating", y = "Number of reviews", title ="Star rating distribution business open/closed")
...
Let's look at the leading business categories in our data set. A business is linked to multiple categories in our dataset, so we have to do a bit of preprocessing, which is simple using dplyr package.
